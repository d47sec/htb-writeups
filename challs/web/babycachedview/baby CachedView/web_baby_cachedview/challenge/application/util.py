...
def serve_screenshot_from(url, domain, width=1000, min_height=400, wait_time=10):
    ...
    driver.get(url) # lúc này nó sẽ thực hiện dns query 1 lần nữa , nên có thể lợi dụng để bypass 
...
def cache_web(url):
    ...
    if scheme not in ['http', 'https']: # yêu cầu url ta cung cấp phải có scheme là http or https nếu không sẽ chặn
    ...
    if is_inner_ipaddress(socket.gethostbyname(domain)): # hàm gethostbyname này sẽ nhận vào 1 domain sau đó trả về IP của domain,
